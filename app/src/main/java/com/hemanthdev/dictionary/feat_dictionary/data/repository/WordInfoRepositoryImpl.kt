package com.hemanthdev.dictionary.feat_dictionary.data.repository

import com.hemanthdev.dictionary.core.util.Resource
import com.hemanthdev.dictionary.feat_dictionary.data.local.WordInfoDao
    ...
    override fun getWordInfo(word: String): Flow<Resource<List<WordInfo>>> {
        return flow {
            ...
            try {
                ...
                emit(
                    Resource.Error(
                        message = "Oops, something went wrong!", data = wordInfos
                    )
                )
            } catch (e: IOException) {
                emit(
